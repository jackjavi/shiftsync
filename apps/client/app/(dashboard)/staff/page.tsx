...
export default function StaffPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => usersService.list({ role: roleFilter || undefined, page, limit: 20 }),
  });
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6 animate-fade-in">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-[var(--text-primary)] font-display">Staff</h1>
          <p className="text-sm text-[var(--text-secondary)] font-nunito mt-1">
            {total > 0 ? `${total} staff member${total !== 1 ? 's' : ''}` : 'All staff across locations'}
          </p>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {staff.map((u) => <StaffCard key={u.id} user={u} />)}
        </div>
      )}
