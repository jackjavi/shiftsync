...
export function ShiftFormModal({ open, onClose, shift }: ShiftFormModalProps) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(isEdit ? 'Shift updated' : 'Shift created', isEdit ? 'Changes saved.' : 'New shift added to schedule.');
      onClose();
    },
    onError: (err) => {
      toast.error(isEdit ? 'Update failed' : 'Create failed', parseApiError(err).message);
    },
  });

  return (
    <Modal open={open} onClose={onClose} title={isEdit ? 'Edit Shift' : 'Create Shift'} size="md">
      <form onSubmit={handleSubmit((d) => mutation.mutate(d))} className="space-y-4" noValidate>
        ...
        <div className="grid grid-cols-2 gap-4">
          <Input label="Start time" type="datetime-local" error={errors.startAt?.message} {...register('startAt')} />
          <Input label="End time"   type="datetime-local" error={errors.endAt?.message}   {...register('endAt')}   />
        </div>
        <div className="grid grid-cols-2 gap-4">
          <Input label="Headcount" type="number" min={1} max={20} error={errors.headcount?.message} {...register('headcount')} />
          <Input label="Edit cutoff (hours)" type="number" min={0} hint="Hours before shift when editing is locked" {...register('editCutoffHours')} />
        </div>
        <Input label="Notes (optional)" placeholder="Any details for this shift…" {...register('notes')} />
