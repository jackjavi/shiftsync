...
import { ThemeButtons } from '@/app/components/theme-buttons';
        ...
        {user && (
          <div className="relative" ref={menuRef}>
            ...
            >
              ...
              <ChevronDown className={cn('w-3.5 h-3.5 text-[var(--text-muted)] transition-transform', menuOpen && 'rotate-180')} />
            </button>
            ...
            {menuOpen && (
              <div className={cn(
                ...
                <div className="px-4 py-3 border-b border-[var(--border)]">
                  <p className="text-sm font-semibold text-[var(--text-primary)] font-nunito truncate">{user.name}</p>
                  <p className="text-xs text-[var(--text-muted)] font-nunito truncate">{user.email}</p>
                  <Badge
                    variant={user.role === 'ADMIN' ? 'premium' : user.role === 'MANAGER' ? 'accent' : 'default'}
