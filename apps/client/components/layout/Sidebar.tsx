...
import {
  Calendar, Users, ArrowLeftRight, BarChart3,
  Bell, FileText, Clock, Zap, ChevronLeft, ChevronRight,
  LayoutDashboard,
} from 'lucide-react';
...
const NAV: NavItem[] = [
  { label: 'Schedule',      href: '/schedule',      icon: Calendar,        description: 'Weekly shift calendar',          },
  { label: 'Shifts',        href: '/shifts',         icon: LayoutDashboard, description: 'All shifts across locations',    },
  { label: 'Staff',         href: '/staff',          icon: Users,           description: 'Staff profiles and skills',      roles: ['ADMIN', 'MANAGER'] },
  { label: 'Swaps',         href: '/swaps',          icon: ArrowLeftRight,  description: 'Swap and drop requests',         },
  { label: 'Overtime',      href: '/overtime',       icon: Clock,           description: 'Hours tracking dashboard',       roles: ['ADMIN', 'MANAGER'] },
  { label: 'Analytics',     href: '/analytics',      icon: BarChart3,       description: 'Fairness and distribution',      roles: ['ADMIN', 'MANAGER'] },
  { label: 'Notifications', href: '/notifications',  icon: Bell,            description: 'Alerts and updates',             },
  { label: 'Audit Log',     href: '/audit',          icon: FileText,        description: 'System change history',          roles: ['ADMIN'] },
];
            ...
            >
              ...
              {!collapsed && (
                <>
                  ...
                  {showBadge && (
                    <Badge variant="danger" className="text-[10px] px-1.5 py-0 h-5">
        ...
        <div className={cn(
          ...
          {!collapsed && (
            <div className="flex-1 min-w-0">
              <p className="text-xs font-semibold text-[var(--text-primary)] font-nunito truncate">{user.name}</p>
              <p className="text-[10px] text-[var(--text-muted)] font-nunito uppercase tracking-wide">{user.role}</p>
            </div>
          )}
        </div>
      )}
