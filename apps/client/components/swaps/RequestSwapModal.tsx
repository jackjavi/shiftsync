...
export function RequestSwapModal({ open, onClose }: RequestSwapModalProps) {
  ...
  const upcomingShifts = myShifts.filter((s) => new Date(s.startAt) > new Date());

  const selectedShift: Shift | undefined = upcomingShifts.find((s) => String(s.id) === shiftId);

  // When swap selected, fetch other staff at the same location with the same skill
  const { data: candidates, isLoading: loadingCandidates } = useQuery({
    queryKey: ['swap-candidates', selectedShift?.id],
    ...
    select: (data) =>
      data.data.filter((u) =>
        u.id !== user?.id &&
        u.skills?.some((us) => us.skillId === selectedShift?.skillId),
      ),
  });
  ...
  return (
    <Modal open={open} onClose={handleClose} title="Request Swap or Drop" size="md">
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          {(['DROP', 'SWAP'] as const).map((t) => (
            ...
            >
              {t === 'DROP'
                ? <Trash2 className={`w-5 h-5 ${type === t ? 'text-[hsl(187_100%_42%)]' : 'text-[var(--text-muted)]'}`} />
                : <ArrowLeftRight className={`w-5 h-5 ${type === t ? 'text-[hsl(187_100%_42%)]' : 'text-[var(--text-muted)]'}`} />
              }
              <div>
        ...
        <div>
          ...
          {loadingShifts ? (
            <div className="flex items-center gap-2 p-3 rounded-lg bg-[var(--surface-elevated)]">
              <Spinner size="sm" /> <span className="text-sm text-[var(--text-muted)] font-nunito">Loading your shifts…</span>
            </div>
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-sm font-semibold text-[var(--text-primary)] font-nunito capitalize">
                      ...
                      {shift.isPremium && <span className="ml-1 text-[hsl(43_95%_56%)]">★</span>}
                    </span>
                  ...
                  <div className="flex items-center gap-3 mt-1">
                    <span className="flex items-center gap-1 text-xs text-[var(--text-muted)] font-nunito">
                      ...
                      {formatInTz(shift.startAt, shift.location?.timezone ?? 'UTC', 'EEE MMM d, h:mm a')}
                    </span>
                    {shift.location && (
                      <span className="flex items-center gap-1 text-xs text-[var(--text-muted)] font-nunito">
        ...
        {type === 'SWAP' && selectedShift && (
          <div>
            ...
            {loadingCandidates ? (
              <div className="flex items-center gap-2 p-3 rounded-lg bg-[var(--surface-elevated)]">
                <Spinner size="sm" /> <span className="text-sm text-[var(--text-muted)] font-nunito">Finding eligible staff…</span>
              </div>
            ) : (
              <Select
                options={[
            ...
            {candidates?.length === 0 && (
              <p className="text-xs text-[hsl(43_95%_56%)] font-nunito mt-1.5">
                No other staff share this skill at this location. Consider a drop request instead.
              </p>
            )}
          </div>
        )}
        ...
        <div>
          ...
          <textarea
            ...
            placeholder={type === 'DROP' ? 'Reason for dropping…' : 'Message for the other staff member…'}
            rows={2}
        ...
        <div className="flex justify-end gap-3 pt-2 border-t border-[var(--border)]">
          ...
          <Button
            ...
            icon={type === 'DROP' ? <Trash2 className="w-4 h-4" /> : <ArrowLeftRight className="w-4 h-4" />}
