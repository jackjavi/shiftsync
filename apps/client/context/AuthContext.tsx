...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const isManager   = useCallback(() => user?.role === 'ADMIN' || user?.role === 'MANAGER', [user]);
  ...
  const canManageLocation = useCallback(
    (locationId: number) => {
      ...
      if (user.role === 'MANAGER') return managedLocationIds.includes(locationId);
      return false;
    },
  ...
  const value = useMemo<AuthContextValue>(
    ...
    [user, token, isLoading, login, logout, isAdmin, isManager, isStaffOnly, canManageLocation, managedLocationIds],
  );
