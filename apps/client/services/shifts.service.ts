...
export interface ShiftFilters {
  locationId?: number;
  skillId?: number;
  staffId?: number; // filter by assigned staff member
  from?: string;
  to?: string;
  isPublished?: boolean;
  page?: number;
...
export const shiftsService = {
  list: (filters?: ShiftFilters) =>
    api
      .get<PaginatedResponse<Shift>>("/shifts", { params: filters })
      .then((r) => r.data),

  // Convenience: get shifts assigned to the currently authenticated user
  myShifts: (params?: {
    from?: string;
    to?: string;
    page?: number;
    ...
    api
      .get<PaginatedResponse<Shift>>("/shifts/my", { params })
      .then((r) => r.data),
  ...
  update: (id: number, data: Partial<CreateShiftFormData>) =>
    api
      .patch<ApiResponse<Shift>>(`/shifts/${id}`, data)
      .then((r) => r.data.data),
  ...
  publish: (id: number) =>
    api
      .post<ApiResponse<Shift>>(`/shifts/${id}/publish`)
      .then((r) => r.data.data),

  unpublish: (id: number) =>
    api
      .post<ApiResponse<Shift>>(`/shifts/${id}/unpublish`)
      .then((r) => r.data.data),
